...
from extended_rospylogs import (
    DEBUG_LEVEL_0,
    DEBUG_LEVEL_1,
    DEBUG_LEVEL_2,
    DEBUG_LEVEL_3,
...
class DataCapture:
    def __init__(self, debugger):
        ...
        rospy.Subscriber(
            name="/data_capture/capture",
            data_class=Bool,
            callback=self.capture_cb,
            queue_size=2,
        )

        self.status_pub = rospy.Publisher(
            name="/data_capture/status", data_class=Status, queue_size=2
        )
        self._status = Status()
        ...
        self.streaming_status_service = rospy.ServiceProxy(
            name="kiwibot/video_streaming_status", service_class=VideoStreamingConf
        )
        ...
        self.csv_header = (
            ...
            + [
                "CAM{}_file".format(cam_label)
                for cam_label, status in self.cameras_status.items()
            ]
            + ["odom_roll[rad]", "odom_pitch[rad]", "odom_yaw[rad]"]
            + ["actuator_steering", "actuator_throttle"]
            + [
                "pepwave_timestamp",
                "wan_upload_speed [Kbs]",
                "wan_download_speed [Kbs]",
            ]
        ...
        if len(usb_devices):
            ...
            if usb_devices[-1][0][1:].split("/")[0] == "dev":
                ...
                self.debugger.debugger(
                    DEBUG_LEVEL_0, "USB Detected: {}".format(device), log_type="info"
                )
        ...
        if self.dest_folder is not None:
            ...
            self.csv_file, log_msg = (
                None,
                "USB device no found or it may have been disconnected",
            )
        ...
        try:  # Create data.csv headers if it not exists
            self.debugger.debugger(
                DEBUG_LEVEL_0,
                "Destination folder: {}".format(self.dest_folder),
                log_type="info",
            )
        ...
        if self.video_record and self.dest_folder is not None:
            self.debugger.debugger(
                DEBUG_LEVEL_0, "video recording enabled", log_type="info"
            )
    ...
    def publish_status(self):
        ...
        self._status.space_left = float(100.0 - self.space_left)
    ...
    def capture_cb(self, data):
        ...
        self.space_left = self.usb_space_left(
            device_path=self.dest_folder, percentage=True
        )
        ...
        if self.space_left <= float(os.getenv("DATA_CAPTURE_MIN_USB_SPACE", 3)):
            self.debugger.debugger(
                DEBUG_LEVEL_0, "Can't record video, USB FULL", log_type="warn"
            )
        ...
        # 2 - Check if the usb device is connected
            ...
            self.debugger.debugger(
                DEBUG_LEVEL_0, "Can't record video, USB is not mounted", log_type="warn"
            )
            self.recording = False
        ...
        # 3 - Check if there's no csv file
            ...
            self.debugger.debugger(
                DEBUG_LEVEL_0,
                "Can't record video, some problems with the USB",
                log_type="warn",
            )
            self.recording = False
        ...
        if self.recording:
            self.debugger.debugger(
                DEBUG_LEVEL_0,
                "Data recording {} started".format(self.capture_id),
                log_type="info",
            )
            if self.video_record and self.dest_folder is not None:
            ...
            self.debugger.debugger(
                DEBUG_LEVEL_0,
                "Data recording {} stopped".format(self.capture_id),
                log_type="info",
            )
    ...
    def get_camera_Status(self):
        ...
        cameras_status = dict(
            map(lambda x: (x.split(":")[0], int(x.split(":")[-1])), cameras_status_str)
        )
    ...
    def write_data(self, images_dict):
        ...
        if self.video_record:
            try:
                ...
                self.debugger.debugger(
                    DEBUG_LEVEL_0,
                    "something happened recording video: {}".format(e),
                    log_type="err",
                )
                self.recording = False
                return
        ...
        if self.write_imgs:
            try:
                ...
                self.debugger.debugger(
                    DEBUG_LEVEL_0,
                    "something happened saving images: {}".format(e),
                    log_type="err",
                )
                self.recording = False
                return
            ...
            cam_file_names = {
                cam_label: "NA" for cam_label in self.cameras_status.keys()
            }
        ...
        if self.write_csv:
            ...
            rows = [
                ...
                + [
                    "cam error" if not cam_stat else cam_file_names[cam_label]
                    for cam_label, cam_stat in self.cameras_status.items()
                ]
                + [
                    self.subs_odometry.roll,
                    self.subs_odometry.pitch,
                    self.subs_odometry.theta,
                    self.subs_actuator_control.steering,
                    self.subs_actuator_control.throttle,
                    self.subs_wan_networks.timestamp,
                    self.subs_wan_networks.speed["upload"],
                    self.subs_wan_networks.speed["download"],
                    self.subs_wan_networks.gps["status"],
                    self.subs_wan_networks.gps["vdop"],
                    self.subs_wan_networks.gps["hdop"],
                    self.subs_wan_networks.gps["pdop"],
            ...
            try:  # Write data and variables in csv file
                ...
                self.debugger.debugger(
                    DEBUG_LEVEL_0,
                    "something happened saving data in csv file",
                    log_type="err",
                )
                self.recording = False
                return
...
def main():
    ...
    try:  # Wait for cameras status response
        main_debugger.debugger(
            DEBUG_LEVEL_0, "Waiting for cameras status response", log_type="info"
        )
        rospy.wait_for_service(service="video_mapping/cameras_status", timeout=20)
    except (rospy.ServiceException, rospy.ROSException), e:
        main_debugger.debugger(
            DEBUG_LEVEL_0, "Did not get cameras response status", log_type="err"
        )
        return 1
    ...
    try:  # Wait for cameras status response
        main_debugger.debugger(
            DEBUG_LEVEL_0,
            "Waiting for video streaming status response",
            log_type="info",
        )
        rospy.wait_for_service(service="kiwibot/video_streaming_status", timeout=20)
    except (rospy.ServiceException, rospy.ROSException), e:
        main_debugger.debugger(
            DEBUG_LEVEL_0, "Did not get video streaming response status", log_type="err"
        )
        return 1
    main_debugger.debugger(
        DEBUG_LEVEL_0, "Got video streaming status service", log_type="info"
    )

    # -------------------------------------------------------------------------
    # Create object for data capture
    BotDataCapture = DataCapture(debugger=main_debugger)
    # Finish and stop node is usb was not detected
    if BotDataCapture.dest_folder is None:
        main_debugger.debugger(
            DEBUG_LEVEL_0, "data capture node killed due to no usb", log_type="err"
        )
        time.sleep(2.0)
        return 0
    ...
    video_map = MultiImagesMemmap(
        mode="r", name="main_stream", memmap_path=os.getenv("MEMMAP_PATH", "/tmp")
    )
    ...
    while not rospy.is_shutdown():
        ...
        if BotDataCapture.recording:
            ...
            images_dict = {
                cam_label: video_map.read(cam_label)
                for cam_label, cam_status in BotDataCapture.cameras_status.items()
                if cam_status
            }
    ...
    if BotDataCapture.video_record:
        ...
        main_debugger.debugger(
            DEBUG_LEVEL_0, "forced video recording stop", log_type="warn"
        )
