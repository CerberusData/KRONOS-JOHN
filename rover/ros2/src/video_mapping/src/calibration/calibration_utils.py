...
from python_utils.utils import FindBotViewCord
from python_utils.utils import get_projection_point_dst
from python_utils.utils import get_projection_point_src
from python_utils.utils import line_intersection
from vision.utils import overlay_image

# =============================================================================
LOCAL_CALIBRATION = int(os.getenv(key="LOCAL_CALIBRATION", default=0)) 
...
def return_pixel_relation(img_src, M, mtx, dist, p1, p2 , p3 , p4, Left_Line, 
    ...
    print("[INFO][EXTRINSIC_RELATION]: Calculating pixel relation parameters")
    ...
    if not ppmx: 
        print("[ERROR][EXTRINSIC_RELATION]: No 'X' axis pixel relation found")
    ...
    H1 = Hy; H2 = 0; H3 = 0
    if not Hy:
        print("[WARNING][EXTRINSIC_RELATION]: Hy=0, No pattern's horizontal component found in projection calculation")
    ...
    while not ppmy and iterations<PATTERN_ITERATION_TRIES and (not H2 or not H3):
        ...
        iterations += 1
        print("\n\t[INFO]: Iteration {} - Warning: No relation found / H1 = {} / H2 = {} / H3 = {} ".format(iterations, H1, H2, H3))
        if not H2: print("\t\t[INFO]: H2 = 0 - No pattern's horizontal component found for hough lines")
        if not H3: print("\t\t[INFO]: H3 = 0 - No pattern's horizontal component found by pixels")
        if LOCAL_CALIBRATION: cv2.imshow(LOCAL_WIN_NAME, img_proj); cv2.waitKey(500)

    if not ppmy:
        print("[ERROR][EXTRINSIC_RELATION]: Horizontal line in pattern no found, check light conditions and try again")
        if LOCAL_CALIBRATION: 
            cv2.waitKey(1000)
            cv2.destroyWindow(LOCAL_WIN_NAME)
        return None
    ...
    print("[INFO][EXTRINSIC_RELATION]: ppx:{}[pix/m], ppy:{}[pix/m], dead_view:{}[m], Hy:{}".format(
...
def find_projection(img_src, mtx, dist, PATTERN_THRESH_TOP, PATTERN_THRESH_BOTTOM, 
    ...
    # Apply Gaussian filter to image
    img_scr_blur = cv2.GaussianBlur(src=img_scr_undistort, 
        ksize=(PATTERN_FILTER_KERNEL*2+1, PATTERN_FILTER_KERNEL*2+1), sigmaX=0)
    ...
    print("[INFO][EXTRINSIC_CALIBRATION]: Looking for pattern")
    while not solid_pattern and iteration < PATTERN_ITERATION_TRIES:
        ...
        for cnt_idx, cnt in enumerate(contours):
            ...
            if ((solidity > 0.12 and solidity < 0.25)):
                solid_pattern = True
                contours = [cnt]
                print("[INFO][EXTRINSIC_CALIBRATION]: Complete partern found")
                ...
                cv2.drawContours(image=img_scr_hsv, contours=[cnt], contourIdx=-1, 
                    color=(255, 255, 255), thickness=-1)
                print("[WARNING][EXTRINSIC_CALIBRATION]: One side of partern found")
                if pattern_sides == 2:
                    solid_pattern = True
                    print("[WARNING][EXTRINSIC_CALIBRATION]: Fragmented partern found")
                    break
                continue
            else:
                cv2.drawContours(image=img_scr_hsv_gui, contours=[cnt], 
        ...
        if not solid_pattern:
            print("[WARNING][EXTRINSIC_CALIBRATION]: No pattern found in iteration {}".format(iteration))
        iteration += 1# Increment iteration

    if not solid_pattern:
        print("[WARNING][EXTRINSIC_CALIBRATION]: No solid pattern found in image source")
        if len(cnt):
