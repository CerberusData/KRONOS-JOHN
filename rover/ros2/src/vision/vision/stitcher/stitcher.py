...
class Stitcher:
    def __init__(self, abs_path="", super_stitcher=False):
        ...
        self.cachedHCR_INV = (
            None  # Spacial Transformation inverse matrix for C and R images
        )
        ...
        self.cachedMCR = (
            None  # Final transformation matrix in pair images C and R for super mode
        )
        ...
        self.x_coord_CR_limit = (
            None  # X coordinate to overlay transparency in pair images C and R
        )
        self.alpha_mask_CR = (
            None  # Alpha mask 1 to over lay transparency in pair images C and R
        )
        self.alpha_mask_CR2 = (
            None  # Alpha mask 2 to over lay transparency in pair images C and R
        )
        ...
        self.cachedHLC_INV = (
            None  # Spacial Transformation inverse matrix for L and C images
        )
        self.warp_LC_size = None  # Size of first stitching  - (type) = (width, height)
        self.out_limitsLC = (
            None  # OUT limits of panoramic stitching in LC Configuration
        )
        ...
        self.cachedMLC = (
            None  # Final transformation matrix in pair images L and C for super mode
        )
        ...
        self.x_coord_LC_limit = (
            None  # X coordinate to overlay transparency in pair images L and C
        )
        self.alpha_mask_LC = (
            None  # Alpha mask 1 to over lay transparency in pair images L and C
        )
        self.alpha_mask_LC2 = (
            None  # Alpha mask 2 to over lay transparency in pair images L and C
        )
        self.source_C_LC_corners = (
            []
        )  # Corners location in panoramic stitching for C image source in LC Configuration
        self.source_L_LC_corners = (
            []
        )  # Corners location in panoramic stitching for L image source in LC Configuration
        ...
        self.cachedHLCR_INV = (
            None  # Spacial Inverse Transformation matrix for L and CR images
        )
        ...
        self.out_limitsLCR = (
            None  # out limits of panoramic stitching in LCR Configuration
        )
        self.in_limitsLCR = (
            None  # in limits of panoramic stitching in LCR Configuration
        )
        ...
        self.x_coord_LCR_limit = (
            None  # X coordinate to overlay transparency in pair images L and CR
        )
        self.alpha_mask_LCR = (
            None  # Alpha mask 1 to over lay transparency in pair images L and CR
        )
        self.alpha_mask_LCR2 = (
            None  # Alpha mask 2 to over lay transparency in pair images L and CR
        )
        ...
        self.LC_CtnL_Mc = (
            None  # L + C -> Mass center of Left contour surface in stitching space
        )
        self.LC_CtnC_Mc = (
            None  # L + C -> Mass center of Center contour surface in stitching space
        )
        self.CR_CtnC_Mc = (
            None  # C + R -> Mass center of Center contour surface in stitching space
        )
        self.CR_CtnR_Mc = (
            None  # C + R -> Mass center of Right contour surface in stitching space
        )
        self.LCR_CtnL_Mc = (
            None  # L + C + R -> Mass center of Left contour surface in stitching space
        )
        self.LCR_CtnC_Mc = None  # L + C + R -> Mass center of Center contour surface in stitching space
        self.LCR_CtnR_Mc = (
            None  # L + C + R -> Mass center of Right contour surface in stitching space
        )

        # ---------------------------------------------------------------------
        # Center of original images in stitching image
        self.LCR_Lc = (
            None  # L + C + R -> Center of left original image in stitching image
        )
        self.LCR_Cc = (
            None  # L + C + R -> Center of Center original image in stitching image
        )
        self.LCR_Rc = (
            None  # L + C + R -> Center of Right original image in stitching image
        )
    ...
    def calibrate_stitcher(self, images, ratio=0.75, reprojThresh=4.0):
        ...
        if self.cachedHLC is None:
            ...
            if MLC is None:
                printlog(
                    msg="Matched no possible for pair center and left images",
                    msg_type="ERROR",
                )
                return None
            else:
                printlog(
                    msg="Pair left and center images matched successfully",
                    msg_type="INFO",
                )
            ...
            self.corners_LC = []
            self.corners_LC.append(
                np.array(get_projection_point_dst((0, 0, 1), self.cachedHLC))
            )
            self.corners_LC.append(
                np.array(
                    get_projection_point_dst((0, imageC.shape[0], 1), self.cachedHLC)
                )
            )
            self.corners_LC.append(
                np.array(
                    get_projection_point_dst((imageC.shape[1], 0, 1), self.cachedHLC)
                )
            )
            self.corners_LC.append(
                np.array(
                    get_projection_point_dst(
                        (imageC.shape[1], imageC.shape[0], 1), self.cachedHLC
                    )
                )
            )
            ...
            self.out_limitsLC, self.in_limitsLC, corners = get_extreme_contour_corners(
                self.corners_LC
            )
        ...
        if self.x_coord_LC_limit is None:
            ...
            for x_coord in range(0, mask_size):
                alpha_mask_LC[0 : alpha_mask_LC.shape[0], x_coord] = float(
                    x_coord
                ) / float(mask_size)
            self.alpha_mask_LC = cv2.flip(alpha_mask_LC[::], 1)
            self.alpha_mask_LC2 = (
                np.ones((imageL.shape[0], mask_size, 3), np.float32)
                - self.alpha_mask_LC
            )
        ...
        # Process super mode
        if self.super_mode:
            Ca = imageL[:, self.x_coord_LC_limit :] / 255.0
            Cb = (
                result_LC[
                    self.source_L_LC_corners[0][1] : self.source_L_LC_corners[2][1],
                    self.x_coord_LC_limit : imageL.shape[1],
                ]
                / 255.0
            )
        ...
        result_LC[
            self.source_L_LC_corners[0][1] : self.source_L_LC_corners[2][1],
            0 : self.source_L_LC_corners[1][0],
        ] = imageL
        ...
        result_LC = result_LC[
            0 : self.out_limitsLC[1] - self.y_offsetLC, 0 : self.out_limitsLC[3]
        ]
        ...
        if self.cachedMLC is None:
            ...
            dst_points = np.array(
                [
                    ...
                    [result_LC.shape[1], 0],
                    [0, result_LC.shape[0]],
                    [result_LC.shape[1], result_LC.shape[0]],
                ],
                dtype=np.float32,
            )
        ...
        result_LC = cv2.warpPerspective(
            src=result_LC, M=self.cachedMLC, dsize=self.pano_size_LC
        )
        ...
        if self.cachedHCR is None:
            ...
            if MCR is None:
                printlog(
                    msg="Matched no possible for pair center and left video",
                    msg_type="ERROR",
                )
                return None
            else:
                printlog(
                    msg="Pair center and left images matched successfully",
                    msg_type="INFO",
                )
            ...
            self.corners.append(
                np.array(get_projection_point_dst((0, 0, 1), self.cachedHCR))
            )
            self.corners.append(
                np.array(
                    get_projection_point_dst((0, imageR.shape[0], 1), self.cachedHCR)
                )
            )
            self.corners.append(
                np.array(
                    get_projection_point_dst((imageR.shape[1], 0, 1), self.cachedHCR)
                )
            )
            self.corners.append(
                np.array(
                    get_projection_point_dst(
                        (imageR.shape[1], imageR.shape[0], 1), self.cachedHCR
                    )
                )
            )
            ...
            self.out_limitsCR, self.in_limitsCR, corners = get_extreme_contour_corners(
                self.corners
            )
        ...
        if self.x_coord_CR_limit is None:
            ...
            for x_coord in range(0, mask_size):
                alpha_mask_CR[0 : alpha_mask_CR.shape[0], x_coord] = float(
                    x_coord
                ) / float(mask_size)
            self.alpha_mask_CR = cv2.flip(alpha_mask_CR[::], 1)
            self.alpha_mask_CR2 = (
                np.ones((imageC.shape[0], mask_size, 3), np.float32)
                - self.alpha_mask_CR
            )
        ...
        # Process super mode
        if self.super_mode:
            Ca = imageC[:, self.x_coord_CR_limit :] / 255.0
            Cb = (
                result_CR[
                    self.source_C_corners[0][1] : self.source_C_corners[2][1],
                    self.x_coord_CR_limit : imageC.shape[1],
                ]
                / 255.0
            )
        ...
        # Select ROI of first panoramic stitching
        result_CR = result_CR[
            0 : self.out_limitsCR[1] - self.y_offsetCR, 0 : self.out_limitsCR[3]
        ]
        ...
        if self.cachedHLCR is None:
            ...
            self.warp_size_LCR = (
                int(result_CR.shape[1] + imageC.shape[1]),
            ...
            # match features between the two images
            MLCR = self.matchKeypoints(
                kpsA=kpsAB,
                kpsB=kpsC,
                featuresA=featuresAB,
                featuresB=featuresC,
                ratio=ratio,
                reprojThresh=reprojThresh,
            )
            ...
            if MLCR is None:
                printlog(
                    msg="Matched no possible for right + center + left stitching mode",
                    msg_type="ERROR",
                )
                return result_CR
            else:
                printlog(
                    msg="Right + center + left stitching matched successfully",
                    msg_type="INFO",
                )
            ...
            for idx, pt in enumerate(self.corners):
                self.corners[idx] = get_projection_point_dst(
                    (pt[0], pt[1], 1), self.cachedHLCR
                )
        ...
        if self.x_coord_LCR_limit is None:
            ...
            for x_coord in range(0, mask_size):
                alpha_mask_LCR[0 : alpha_mask_LCR.shape[0], x_coord] = float(
                    x_coord
                ) / float(mask_size)
        ...
        result_LCR = cv2.warpPerspective(
            src=result_CR, M=self.cachedHLCR, dsize=self.warp_size_LCR
        )
        # Process with super mode
        if self.super_mode:
            Ca = imageL[:, self.x_coord_LCR_limit :] / 255.0
            Cb = (
                result_LCR[
                    self.source_L_corners[0][1] : self.source_L_corners[2][1],
                    self.x_coord_LCR_limit : imageL.shape[1],
                ]
                / 255.0
            )
        ...
        result_LCR[
            ...
            self.source_L_corners[0][0] : self.source_L_corners[1][0],
        ] = imageL
        ...
        # Select ROI of first panoramic stitching
        result_LCR = result_LCR[
            0 : self.out_limitsLCR[1] - self.y_offsetLCR, 0 : self.out_limitsLCR[3]
        ]
        ...
        if self.cachedMLCR is None:
            ...
            dst_points = np.array(
                [
                    ...
                    [result_LCR.shape[1], result_LCR.shape[0]],
                ],
                dtype=np.float32,
            )
        ...
        printlog(msg="perspective transformation matrix calculated", msg_type="INFO")
        printlog(msg="Stitching matcher calibrated successfully", msg_type="INFO")

        # --------------------------------------------------------------------
        # Process with super mode
        result_LCR = cv2.warpPerspective(
            src=result_LCR, M=self.cachedMLCR, dsize=self.pano_size
        )
        ...
        if save_file:
            self.save_stitcher(abs_path=self.path_config_file)
        ...
        if imageR is None and imageC is not None and imageL is not None:
            ...
            if not self.cachedHLC is None:
                ...
                if self.super_mode:
                    ...
                    Cb = (
                        result_LC[
                            self.source_L_LC_corners[0][1] : self.source_L_LC_corners[
                                2
                            ][1],
                            self.x_coord_LC_limit : imageL.shape[1],
                        ]
                    ...
                    if (
                        self.alpha_mask_LC.shape[1] != Ca.shape[1]
    ...
    def save_stitcher(self, abs_path="", quite=False):
        ...
        try:
            ...
            if not quite:
                printlog(
                    msg="Stitcher configuration saved: {}".format(
                        os.path.join(abs_path)
                    ),
                    msg_type="INFO",
                )
            ...
            printlog(
                msg="something bad occurred saving stitcher configuration file: {}".format(
                    e
                ),
                msg_type="ERROR",
            )
    ...
    def load_stitcher(self, source_path, quiet=False):
        ...
        try:
            ...
            if not quiet:
                printlog(msg="stitcher configuration loaded", msg_type="INFO")
            ...
            printlog(msg="loading stitcher: {}".format(e), msg_type="ERROR")
            printlog(
                msg="parameters established as None and empty arrays, please check configuration file",
                msg_type="WARN",
            )
    ...
    def draw_stitched_projections(self, img_src):
        ...
        if (
            self.stitch_mode[0] == 1
            and self.stitch_mode[1] == 1
            and self.stitch_mode[2] == 1
        ):
            ...
            cv2.putText(
                img_src,
                "LL",
                (self.LCR_Lc[0] + x_offset, 20 + y_offset),
        ...
        # For stitching mode Left Camera + Center Camera = "LC"
            ...
            self.stitch_mode[0] == 1
            and self.stitch_mode[1] == 1
            and self.stitch_mode[2] == 0
        ):
            if self.LC_CtnL is not None:
                for pt in self.LC_CtnL:
                    cv2.circle(img_src, tuple(pt), 1, (255, 255, 0), -1)
            pts_rr = np.array(
                [[self.LC_Lc[0], 0], [self.LC_Lc[0] + 10, 20], [self.LC_Lc[0] + 20, 0]],
                np.int32,
            )
            cv2.putText(
                img_src,
                "LL",
                (self.LC_Lc[0], 20 + y_offset),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 0),
                5,
            ...
            if self.LC_CtnC is not None:
                for pt in self.LC_CtnC:
                    cv2.circle(img_src, tuple(pt), 1, (0, 255, 0), -1)
            pts_rr = np.array(
                [[self.LC_Cc[0], 0], [self.LC_Cc[0] + 10, 20], [self.LC_Cc[0] + 20, 0]],
                np.int32,
            )
            cv2.putText(
                img_src,
                "C",
                (self.LC_Cc[0] + 5, 20 + y_offset),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 0),
                5,
        ...
        # For stitching mode Center Camera + Right Camera = "CR"
            ...
            self.stitch_mode[0] == 0
            and self.stitch_mode[1] == 1
            and self.stitch_mode[2] == 1
        ):
            if self.CR_CtnC is not None:
                for pt in self.CR_CtnC:
                    cv2.circle(img_src, tuple(pt), 1, (0, 255, 0), -1)
            pts_rr = np.array(
                [[self.CR_Cc[0], 0], [self.CR_Cc[0] + 10, 20], [self.CR_Cc[0] + 20, 0]],
                np.int32,
            )
            cv2.putText(
                img_src,
                "C",
                (self.CR_Cc[0] + 5, 20 + y_offset),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 0),
                5,
            ...
            if self.CR_CtnR is not None:
                for pt in self.CR_CtnR:
                    cv2.circle(img_src, tuple(pt), 1, (0, 0, 255), -1)
            pts_rr = np.array(
                [[self.CR_Rc[0], 0], [self.CR_Rc[0] + 10, 20], [self.CR_Rc[0] + 20, 0]],
                np.int32,
            )
            cv2.putText(
                img_src,
                "RR",
                (self.CR_Rc[0], 20 + y_offset),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 0),
                5,
    ...
    def get_coord_source(
        ...
        if (
            ...
            if self.vir_pan_val > 100 or self.vir_pan_val < -100:
                ...
                if coord[1] <= dl or coord[1] > self.Window_size[1] - dl:
                    printlog(msg="LCR - click out of boundaries", msg_type="WARN")
                    return None, None, None
                coord[1] -= dl
                coord[1] = int(
                    self.pano_size_LCR[1]
                    * float(coord[1])
                    / float(r * self.pano_size_LCR[1])
                )
                ...
                if self.vir_pan_val <= 0:
                    x_offset = int(
                        ...
                        + self.vir_pan_val
                        * (self.LCR_Cc[0] - self.Window_size[0] / 2)
                        / 100.0
                    )
                if self.vir_pan_val > 0:
                    x_offset = int(self.LCR_Cc[0] - self.Window_size[0] / 2) + int(
                        (
                            self.vir_pan_val
                            * (
                                self.pano_size_LCR[0]
                                - int(self.LCR_Cc[0] + self.Window_size[0] / 2)
                            )
                        )
                        / 100.0
                    )
            ...
            if self.LCR_CtnL is not None:  # If there's left camera calibration
                if (
                    cv2.pointPolygonTest(np.array(self.LCR_CtnL), tuple(coord), True)
                    >= 0
                ):
                    Origin = "LL"
                    if self.cachedMLCR_INV is not None:
                        ...
                        Coord_src = get_projection_point_dst(
                            coords_src=(Coord_src[0], Coord_src[1], 1),
                            M=self.cachedMLCR_INV,
                        )
                        Coord_src[1] -= self.source_L_corners[0][1]
                    if self.cachedMLCR_INV is None:
                        printlog(
                            msg="LCR - Variable cachedMLCR_INV no defined",
                            msg_type="WARN",
                        )
            ...
            if self.LCR_CtnC is not None:  # If there's center camera calibration
                if (
                    cv2.pointPolygonTest(np.array(self.LCR_CtnC), tuple(coord), True)
                    >= 0
                ):
                    Origin = "C"
                    if (self.cachedMLCR_INV is not None) or (
                        self.cachedHLCR_INV is not None
                    ):
                        Coord_src = np.array(Coord_stitch).copy()
                        Coord_src[1] += self.in_limitsLCR[0]
                        Coord_src = get_projection_point_dst(
                    ...
                    if self.cachedHLCR_INV is None:
                        printlog(
                            msg="LCR - Variable cachedHLCR_INV no defined",
                            msg_type="WARN",
                        )
                    Coord_src = [int(Coord_src[0]), int(Coord_src[1])]
                    return Origin, Coord_src, Coord_stitch
            ...
            if self.LCR_CtnR is not None:  # If there's right camera calibration
                if (
                    cv2.pointPolygonTest(np.array(self.LCR_CtnR), tuple(coord), True)
                    >= 0
                ):
                    Origin = "RR"
                    if (
                        (self.cachedMLCR_INV is not None)
                        or (self.cachedHLCR_INV is not None)
                        or (self.cachedHCR_INV is not None)
                    ):
                        Coord_src = np.array(Coord_stitch).copy()
                        Coord_src[1] += self.in_limitsLCR[0]
                        Coord_src = get_projection_point_dst(
                    ...
                    if self.cachedHCR_INV is None:
                        printlog(
                            msg="LCR - Variable cachedHCR_INV no defined",
                            msg_type="WARN",
                        )
                    Coord_src = [int(Coord_src[0]), int(Coord_src[1])]
                    return Origin, Coord_src, Coord_stitch
        ...
        # For stitching mode Left Camera + Center Camera = "LC"
            ...
            if self.vir_pan_val > 0 or self.vir_pan_val < -100:
                printlog(msg="Virtual pan exceeds ranges for LC mode", msg_type="WARN")
                return None, None, None
            x_offset = int(
                (100 + self.vir_pan_val)
                * (self.pano_size_LC[0] - self.Window_size[0])
                / 100.0
            )
            coord[0] = (coord[0] * self.Window_size[0]) + x_offset
            ...
            if self.LC_CtnL is not None:  # If there's left camera calibration
                if (
                    cv2.pointPolygonTest(np.array(self.LC_CtnL), tuple(coord), True)
                    >= 0
                ):
                    Origin = "LL"
                    if self.cachedMLC_INV is not None:
                        Coord_src = get_projection_point_dst(
                            coords_src=(coord[0], coord[1], 1), M=self.cachedMLC_INV
                        )
                        Coord_src[1] -= self.source_L_LC_corners[0][1]
                    if self.cachedMLC_INV is None:
                        printlog(
                            msg="Variable cachedMLC_INV no defined", msg_type="WARN"
                        )
                    Coord_src = [int(Coord_src[0]), int(Coord_src[1])]
                    return Origin, Coord_src, Coord_stitch
            ...
            if self.LC_CtnC is not None:  # If there's center camera calibration
                if (
                    cv2.pointPolygonTest(np.array(self.LC_CtnC), tuple(coord), True)
                    >= 0
                ):
                    Origin = "C"
                    if (self.cachedMLC_INV is not None) or (
                        self.cachedHLC_INV is not None
                    ):
                        Coord_src = get_projection_point_dst(
                            coords_src=(coord[0], coord[1], 1), M=self.cachedMLC_INV
                        )
                        Coord_src = get_projection_point_dst(
                            coords_src=(Coord_src[0], Coord_src[1], 1),
                            M=self.cachedHLC_INV,
                        )
                    if self.cachedHLC_INV is None:
                        printlog(
                            msg="Variable cachedHLC_INV no defined", msg_type="WARN"
                        )
                    if self.cachedMLC_INV is None:
                        printlog(
                            msg="Variable cachedMLC_INV no defined", msg_type="WARN"
        ...
        # For stitching mode Center Camera + Right Camera = "CR"
            ...
            if self.vir_pan_val < 0 or self.vir_pan_val > 100:
                printlog(msg="Virtual pan exceeds ranges for CR mode", msg_type="WARN")
                return None, None, None

            x_offset = int(
                (
                    self.vir_pan_val
                    * (int(self.pano_size_CR[0] - self.Window_size[0]))
                    / 100.0
                )
            )
            coord[0] = (coord[0] * self.Window_size[0]) + x_offset
            ...
            if self.CR_CtnC is not None:  # If there's center camera calibration
                if (
                    cv2.pointPolygonTest(np.array(self.CR_CtnC), tuple(coord), True)
                    >= 0
                ):
                    Origin = "C"
                    if self.cachedMCR_INV is not None:
                        Coord_src = get_projection_point_dst(
                            (coord[0], coord[1], 1), self.cachedMCR_INV
                        )
                        Coord_src[1] -= self.source_C_corners[0][1]
                    if self.cachedMCR_INV is None:
                        printlog(
                            msg="CR - Variable cachedMCR_INV no defined",
                            msg_type="WARN",
                        )
                    Coord_src = [int(Coord_src[0]), int(Coord_src[1])]
                    return Origin, Coord_src, Coord_stitch
            ...
            if self.CR_CtnR is not None:  # If there's right camera calibration
                if (
                    cv2.pointPolygonTest(np.array(self.CR_CtnR), tuple(coord), True)
                    >= 0
                ):
                    Origin = "RR"
                    if (self.cachedMCR_INV is not None) or (
                        self.cachedHCR_INV is not None
                    ):
                        Coord_src = get_projection_point_dst(
                            (coord[0], coord[1], 1), self.cachedMCR_INV
                        )
                        Coord_src = get_projection_point_dst(
                            (Coord_src[0], Coord_src[1], 1), self.cachedHCR_INV
                        )
                    if self.cachedMCR_INV is None:
                        printlog(
                            msg="CR - Variable cachedMCR_INV no defined",
                    ...
                    if self.cachedHCR_INV is None:
                        printlog(
                            msg="CR - Variable cachedHCR_INV no defined",
                            msg_type="WARN",
                        )
                    Coord_src = [int(Coord_src[0]), int(Coord_src[1])]
                    return Origin, Coord_src, Coord_stitch
    ...
    def set_proj_cnts(self, cams_params=None):
        ...
        # Print information
        printlog(
            msg="Re-calculation surface projection contour in stitching space",
            msg_type="INFO",
        )

        if cams_params is None:
            printlog(
                msg="No camera parameters specified, contours projection no possible",
                msg_type="WARN",
            )
            return
        else:
            self.PP_CntL = (
                cams_params["LL"]["waypoint_area"]
                if cams_params["LL"] is not None
                else None
            )
            self.PP_CntR = (
                cams_params["RR"]["waypoint_area"]
                if cams_params["RR"] is not None
                else None
            )
            self.PP_CntC = (
                cams_params["C"]["waypoint_area"]
                if cams_params["C"] is not None
                else None
            )

            if self.PP_CntL is None:
                printlog(msg="Parameters for left camera no defined", msg_type="WARN")
            if self.PP_CntC is None:
                printlog(msg="Parameters for Center camera no defined", msg_type="WARN")
            if self.PP_CntR is None:
                printlog(msg="Parameters for Right camera no defined", msg_type="WARN")
            ...
            if (self.cachedHLC is not None) and (self.PP_CntC is not None):
                ...
                for idx, pt in enumerate(self.LC_CtnC):
                    pt = get_projection_point_dst((pt[0], pt[1], 1), self.cachedHLC)
                    self.LC_CtnC[idx] = get_projection_point_dst(
                        (pt[0], pt[1], 1), self.cachedMLC
                    )
            ...
            # For Left contour = L
            if (
                (self.y_offsetLC is not None)
                and (self.cachedMLC is not None)
                and (self.PP_CntL is not None)
            ):
                self.LC_CtnL = np.array(self.PP_CntL).copy()
                for idx, pt in enumerate(self.LC_CtnL):
                    pt[1] += self.source_L_LC_corners[0][1]
                    self.LC_CtnL[idx] = get_projection_point_dst(
                        (pt[0], pt[1], 1), self.cachedMLC
                    )
            ...
            if self.y_offsetLC is None:
                printlog(msg="CR - No y_offsetLC variable defined", msg_type="ERROR")
            if self.cachedHLC is None:
                printlog(msg="CR - No cachedHLC variable defined", msg_type="ERROR")
            if self.cachedMLC is None:
                printlog(msg="CR - No cachedMLC variable defined", msg_type="ERROR")
            if self.PP_CntL is None:
                printlog(
                    msg="LC - Parameters for left camera no defined", msg_type="ERROR"
                )
            if self.PP_CntC is None:
                printlog(
                    msg="LC - Parameters for Center camera no defined", msg_type="ERROR"
                )

            # Report process information
            if not self.LC_CtnC is None and not self.LC_CtnL is None:
                printlog(
                    msg="LC - Projections in stitching space calculated",
                    msg_type="INFO",
                )
            elif not self.LC_CtnC is None or not self.LC_CtnL is None:
                printlog(
                    msg="LC - Projections in stitching space partially calculated",
                    msg_type="WARN",
                )
            else:
                printlog(
                    msg="LC - Projections in stitching space no calculated",
                    msg_type="ERROR",
                )
            ...
            # For Right contour = R
            if (
                self.cachedHCR is not None
                and self.cachedMCR is not None
                and self.PP_CntR is not None
            ):
                self.CR_CtnR = np.array(self.PP_CntR).copy()
                for idx, pt in enumerate(self.CR_CtnR):
                    pt = get_projection_point_dst((pt[0], pt[1], 1), self.cachedHCR)
                    self.CR_CtnR[idx] = get_projection_point_dst(
                        (pt[0], pt[1], 1), self.cachedMCR
                    )
            ...
            # For Center contour = C
            if (
                self.y_offsetCR is not None
                and self.cachedMCR is not None
                and self.PP_CntC is not None
            ):
                self.CR_CtnC = np.array(self.PP_CntC).copy()
                for idx, pt in enumerate(self.CR_CtnC):
                    pt[1] += self.source_C_corners[0][1]
                    self.CR_CtnC[idx] = get_projection_point_dst(
                        (pt[0], pt[1], 1), self.cachedMCR
                    )
            ...
            # Print any error
            if self.cachedHCR is None:
                printlog(msg="CR - No cachedHCR variable defined", msg_type="ERROR")
            if self.cachedMCR is None:
                printlog(msg="CR - No cachedMCR variable defined", msg_type="ERROR")
            if self.y_offsetCR is None:
                printlog(msg="CR - No y_offsetCR variable defined", msg_type="ERROR")
            if self.PP_CntC is None:
                printlog(
                    msg="CR - Parameters for Center camera no defined", msg_type="ERROR"
                )
            if self.PP_CntR is None:
                printlog(
                    msg="CR - Parameters for Right camera no defined", msg_type="ERROR"
                )

            # Report process information
            if self.CR_CtnC is not None and self.CR_CtnR is not None:
                printlog(
                    msg="CR - Projections in stitching space calculated",
                    msg_type="INFO",
                )
            elif self.CR_CtnC is not None or self.CR_CtnR is not None:
                printlog(
                    msg="CR - Projections in stitching space partially calculated",
                    msg_type="WARN",
                )
            else:
                printlog(
                    msg="CR - Projections in stitching space no calculated",
                    msg_type="ERROR",
                )
            ...
            if (
                (self.cachedHLCR is not None)
                and (self.cachedHCR is not None)
                and (self.cachedMLCR is not None)
            ):

                # For Right contour = R
                if (
                    (self.PP_CntR is not None)
                    and (self.cachedHCR is not None)
                    and (self.cachedHLCR is not None)
                    and (self.cachedMLCR is not None)
                ):
                    self.LCR_CtnR = np.array(self.PP_CntR).copy()
                    for idx, pt in enumerate(self.LCR_CtnR):
                        pt = get_projection_point_dst((pt[0], pt[1], 1), self.cachedHCR)
                        pt = get_projection_point_dst(
                            (pt[0], pt[1], 1), self.cachedHLCR
                        )
                        pt = get_projection_point_dst(
                            (pt[0], pt[1], 1), self.cachedMLCR
            ...
            if self.PP_CntL is None:
                printlog(
                    msg="LCR - Parameters for Left camera no defined", msg_type="ERROR"
                )
            if self.PP_CntC is None:
                printlog(
                    msg="LCR - Parameters for Center camera no defined",
                    msg_type="ERROR",
                )
            if self.PP_CntR is None:
                printlog(
                    msg="LCR - Parameters for Right camera no defined", msg_type="ERROR"
                )

            # Report process information
            if (
                self.LCR_CtnL is not None
                and self.LCR_CtnC is not None
                and self.LCR_CtnR is not None
            ):
                printlog(
                    msg="LCR - Projections in stitching space calculated",
                    msg_type="INFO",
                )
            elif (
                self.LCR_CtnL is not None
                or self.LCR_CtnC is not None
                or self.LCR_CtnR is not None
            ):
                printlog(
                    msg="LCR - Projections in stitching space partially calculated",
                    msg_type="WARN",
                )
            else:
                printlog(
                    msg="LCR - Projections in stitching space no calculated",
                    msg_type="ERROR",
                )
    ...
    def get_virtual_pan(self, img_src, pan_value=0):
        ...
        if (
            ...
            if pan_value >= 0 and pan_value <= 100:
                x_offset = int(
                    (
                        pan_value
                        * (pano_size[1] - int(self.LCR_Cc[0] + self.Window_size[0] / 2))
                    )
                    / 100.0
                )
            ...
            if pan_value > 100 or pan_value < -100:
                img_dts = self.image_resize(
                    image=img_src,
                    width=self.Window_size[0],
                    height=self.Window_size[1],
                    inter=self.warp_flags,
                )
                cv2.rectangle(
                    img_src, (0, 0), (pano_size[1], pano_size[0]), (0, 255, 0), 2
                )
            else:
            ...
            return cv2.resize(
                src=img_dts, dsize=self.Window_size, interpolation=self.warp_flags
            )
        ...
        # For stitching mode Left Camera + Center Camera = "LC"
            ...
            if pan_value < 0 and pan_value >= -100:
                x_offset = int(
                    (pan_value * (int(pano_size[1] - self.Window_size[0])) / 100.0)
                )
