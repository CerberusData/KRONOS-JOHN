...
def find_usb_ports(cameras):
    ...
    for cam in cameras:
        try:
            ...
            printlog(
                msg="----- ERROR READING VIDEO DEVICE ----- (Error: {})".format(e),
                msg_type="ERROR",
            )
        ...
        path = path[0 : path.find("video4linux")].split("/")[
            -2
        ]  # get last item where the port is explicit
...
class CameraHandler:
    def __init__(self, device_number, cam_config, cam_label):
        ...
        self.cam_device_path = "/dev/video{}".format(
            device_number
        )  # Camera video device
        ...
        self.cap = None
        self.video_flip = (
            self.cam_config["FLIP"] if "FLIP" in self.cam_config.keys() else 0
        )
        ...
        if self.cap.isOpened():
            ...
            try:  # Grab frame from camera and assing properties
                ...
                if self.grabbed:
                    ...
                    self.video_format = decode_fourcc(self.cap.get(cv2.CAP_PROP_FOURCC))
                    printlog(
                        msg="(GOT VIDEO) {}: DEVICE:{} - SIZE:{}X{} - FPS:{}"
                    ...
                    if self.cam_config["FPS"] > float(self.cap.get(cv2.CAP_PROP_FPS)):
                        printlog(
                            "{}: FRAME RATE AS {} NOT SUPPORTED, SETTED "
                            "AS {}".format(
                                self.cam_label,
                                int(self.cam_config["FPS"]),
                                int(self.cap.get(cv2.CAP_PROP_FPS)),
                    ...
                    printlog(
                        "{}:{} did not capture first image, possibly not "
            ...
            # If not possible open camera port print exception
                ...
                printlog(
                    "Something error ocurred reading frames from camera "
                    "{} (device {}) -> {}".format(
                        self.cam_label, self.cam_device_number, e
                    ),
                    msg_type="ERROR",
                )
    ...
    def set_properties(self):
        ...
        for cam_prop_key in self.cam_config:
            ...
            if type(self.cam_config[cam_prop_key]) is list:
                self.cap.set(
                    self.cam_config[cam_prop_key][0], self.cam_config[cam_prop_key][1]
                )

                time.sleep(0.2)
                # Check that parameters was setted
                prop_val = self.cap.get(self.cam_config[cam_prop_key][0])
                if prop_val != self.cam_config[cam_prop_key][1]:
                    if cam_prop_key == "FOURCC":
                        printlog(
                            "{} - DEVICE:{} CAN NOT SET PIXEL FORMAT AS"
                            " {}".format(
                                self.cam_label,
                                self.cam_device_number,
        ...
        if self.grabbed and self.cap is not None:
            ...
            try:  # Grab a frame
                ...
                if not self.grabbed:  # If camera was disconnected
                    printlog(
                        "Camera: {} HOT Unplugged!".format(self.cam_label),
                        msg_type="ERROR",
                    )
                ...
                self.image = (
                    ...
                    if (
                        cap_image.shape[0] != self.video_height
                        or cap_image.shape[1] != self.video_width
                    )
                    else cap_image
                )
                ...
                printlog(
                    "Camera: {} error while reading: {}".format(self.cam_label, e),
                    msg_type="ERROR",
                )
                self.cap = None
                return self.image
...
class CamerasCaptures:
    def __init__(self, cams_config):
        ...
        if not len(self.usb_ports_cameras):
            ...
            printlog(
                "Video device numbers detected (no enough): {}".format(
        ...
        self.cameras_status = [
            "{}:{}".format(cam_key, int(self.camera_handlers[cam_key].grabbed))
    ...
    def get_cameras_status(self):
        ...
        return {
            key: True if not cam_handler.cap is None else False
