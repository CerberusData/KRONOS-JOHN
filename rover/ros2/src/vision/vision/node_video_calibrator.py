...
class CalibratorPublishers(Node):
    def __init__(self):
        ...
        self._VISION_CAL_UNWARPED_WIDTH = int(
            os.getenv(key="VISION_CAL_UNWARPED_WIDTH", default=200)
        )
        self._VISION_CAL_UNWARPED_HEIGHT = int(
            os.getenv(key="VISION_CAL_UNWARPED_HEIGHT", default=360)
        )
        self._VISION_CAL_UNWARPED_SIZE = (
            self._VISION_CAL_UNWARPED_WIDTH,
            self._VISION_CAL_UNWARPED_HEIGHT,
        )
        self._VISION_CAL_PAT_VER = float(
            os.getenv(key="VISION_CAL_PAT_VER", default=1.0)
        )
        self._VISION_CAL_PAT_HOZ = float(
            os.getenv(key="VISION_CAL_PAT_HOZ", default=1.0)
        )
        self._VISION_CAL_PAT_TH_TOP = int(
            os.getenv(key="VISION_CAL_PAT_TH_TOP", default=15)
        )
        self._VISION_CAL_PAT_TH_BOTTOM = int(
            os.getenv(key="VISION_CAL_PAT_TH_BOTTOM", default=15)
        )
        self._VISION_CAL_SHOW_TIME = int(
            os.getenv(key="VISION_CAL_SHOW_TIME", default=5)
        )
        self._VISION_CAL_PAT_ITE_TRIES = int(
            os.getenv(key="VISION_CAL_PAT_ITE_TRIES", default=20)
        )
        self._VISION_CAL_DAYS_REMOVE = int(
            os.getenv(key="VISION_CAL_DAYS_REMOVE", default=20)
        )
        self._VISION_CAL_DAYS_OUT = int(
            os.getenv(key="VISION_CAL_DAYS_OUT", default=20)
        )
        self._VISION_CAL_SHOW_LOCAL = int(
            os.getenv(key="VISION_CAL_SHOW_LOCAL", default=0)
        )
        self._HSVI = {
        ...
        self.sub_cams_status = self.create_subscription(
            msg_type=CamerasStatus,
            topic="video_streaming/cams_status",
        ...
        self.sub_calibration = self.create_subscription(
            msg_type=String,
            topic="video_calibrator/calibrate_cam",
            callback=self.cb_calibrate,
            qos_profile=5,
            callback_group=self.callback_group,
        )
        ...
        self.sub_img_to_calibrate = self.create_subscription(
            msg_type=Image,
            topic="video_calibrator/calibrate_img",
            callback=self.cb_img_to_calibrate,
            qos_profile=5,
            callback_group=self.callback_group,
        )
    ...
    def cb_img_to_calibrate(self, msg):
        ...
        try:
            ...
            printlog(
                msg="error while getting data from video_calibrator/"
                "calibrate_img, {}".format(e),
                msg_type="ERROR",
            )
    ...
    def cb_calibrate(self, msg):
        ...
        if cam_label not in self._cams_status.keys():
            printlog(
                msg="{} is not a camera label available in extrinsic {}"
                "".format(cam_label, self._cams_status.keys()),
                msg_type="ERROR",
            )
            time.sleep(10)
            return
        ...
        if self._LOCAL_RUN:
            ...
            if self.cam_img is None:
                printlog(
                    msg="Timeout getting image from topic, camera no calibrated",
                    msg_type="ERROR",
                )
                self.calibrating_stat = False
                return False
            ...
            if self.intrinsic.mtx is not None and not self._LOCAL_RUN:
                ...
                if (
                    self.cam_img.shape[1] != self.intrinsic.image_width
                    and self.cam_img.shape[0] != self.intrinsic.image_height
                ):
                    self.cam_img = cv2.resize(
                        self.cam_img,
                        (self.intrinsic.image_width, self.intrinsic.image_height),
                    )

                # Undistord image if intrinsic calibration
                self.cam_img = cv2.remap(
                    self.cam_img,
                    self.intrinsic.map1,
                    self.intrinsic.map2,
                    cv2.INTER_LINEAR,
        ...
        try:
            ...
            if self.intrinsic.mtx is not None:
                self.calibrate_distances(img_src=self.cam_img, camera_label=cam_label)
            else:
                self.pub_log(
                    msg="No intrinsic calibration to perform extrinsic",
                    msg_type="ERROR",
                )
            ...
            printlog(msg="Exception error: {}".format(e), msg_type="ERROR")
        except:
            printlog(
                msg="Unexpected error: {}".format(sys.exc_info()[0]), msg_type="ERROR"
            )
            raise
    ...
    def publish_extrinsic(self, cam_label, extrinsic_data):
        ...
        try:
            ...
            printlog(
                msg="Error publishing extrinsic calibration"
                "trought topic, {}".format(e),
                msg_type="ERROR",
            )
            return False
        ...
        if ext_cal is not None:
            try:
                ...
                surf_relation = pixel_relation(
                    ...
                    p3=ext_cal["p3"],
                    p4=ext_cal["p4"],
                    Left_Line=ext_cal["Left_Line"],
                    Right_Line=ext_cal["Right_Line"],
                    Hy=ext_cal["Hy"],
                ...
                extrinsic_abs_path = os.path.join(self._CONF_PATH, extrinsic_file_name)
                extrinsic_vision_params = {
                    "M": M.tolist(),
                    "vp": [int(ext_cal["vp"][0]), int(ext_cal["vp"][1])],
                    "p1": list(ext_cal["p1"]),
                    "p2": list(ext_cal["p2"]),
                    "p3": list(ext_cal["p3"]),
                    "p4": list(ext_cal["p4"]),
                ...
                self.save_extrinsic(
                    file_path=extrinsic_abs_path, data=extrinsic_vision_params
                )

                # Set calibration image and show it to pilots console
                mask_pro, mask_src = create_ruler_mask(
                    flag_img=self._flag_image, extrinsic_params=extrinsic_vision_params
                )
                self.calibration_image = overlay_image(
                    l_img=img_src.copy(), s_img=mask_src, pos=(0, 0), transparency=1
                )
                calibration_image = self.calibration_image.copy()

                # overlay with rules in wrapped perspective
                cal_img_pro = cv2.warpPerspective(
                    src=self.calibration_image, M=M, dsize=self._UNWARPED_SIZE
                )
                ruler_projected_orig = overlay_image(
                    l_img=cal_img_pro, s_img=mask_pro, pos=(0, 0), transparency=1
                )
                ruler_projected = cv2.resize(
                    src=cv2.rotate(
                        src=ruler_projected_orig, rotateCode=cv2.ROTATE_90_CLOCKWISE
                    ),
                    dsize=self._VIDEO_SIZE,
                )
                ...
                self.publish_extrinsic(
                    cam_label=camera_label, extrinsic_data=extrinsic_vision_params
                )

                # Report successfully calibration
                self.pub_log(
                    msg="Calibration pattern for CAM{} succeed!".format(camera_label),
                    msg_type="OKGREEN",
                )
                ...
                printlog(
                    msg="Calibration pattern no found! ({})".format(e), msg_type="ERROR"
                )
                return False

        else:
            self.pub_log(msg="Calibration pattern no found!", msg_type="ERROR")
            return False
    ...
    def save_extrinsic(self, file_path, data):
        ...
        try:  # Save the calibration data in file
            ...
            printlog(
                msg="Camera extrinsic calibration saved {}".format(file_path),
                msg_type="INFO",
            )

        except IOError as e:  # Report any error saving de data
            printlog(
                msg="Problem saving camera extrinsic calibration: {}".format(e),
                msg_type="ERROR",
            )
