...
class MappingNode(Node, Thread):
    def __init__(self):
        ...
        self._FR_STREAMING_OPTIMIZER = int(
            os.getenv(key="FR_STREAMING_OPTIMIZER", default=0)
        )
        ...
        # the cameras
        self.cams_config = read_cams_configuration(
            FILE_NAME="cams_conf_local.yaml" if self._LOCAL_RUN else "cams_conf.yaml",
        ...
        if self._STITCHER:
            self.stitcher = (
                Stitcher(
                    ...
                    super_stitcher=self._STITCHER_SUP_MODE,
                )
                if self._STITCHER
                else None
            )
        ...
        if self._FR_AGENT and "C" in self.cams_config.keys():
            if self.cams_config["C"]["TOPIC"] != "None":
                ...
                printlog(
                    msg="No topic for video camera C streaming topic"
        ...
        self.pb_img_to_calibrate = self.create_publisher(
            Image,
            "video_calibrator/calibrate_img",
            5,
            callback_group=self.callback_group,
        )
        self.sub_calibration = self.create_subscription(
            ...
            callback=self.cb_send_img_calibrate,
            qos_profile=5,
            callback_group=self.callback_group,
        )
        ...
        self.sub_calibrator_img_res = self.create_subscription(
            msg_type=Image,
            topic="video_calibrator/calibrate_img_result",
            callback=self.cb_cal_img_result,
            qos_profile=5,
            callback_group=self.callback_group,
        )

        # ---------------------------------------------------------------------
        self.tm_pub_cams_status = self.create_timer(1.0, self.cb_cams_status)
    ...
    def cb_cams_status(self):
        ...
        msg = CamerasStatus()
        msg.cams_status = [
            str("{}:{}".format(cam_key, int(cam_status)))
    ...
    def cb_cal_img_result(self, msg):
        ...
        try:
            ...
            printlog(
                ...
                "calibrate_img_result, {}".format(e),
                msg_type="ERROR",
            )

        time.sleep(int(os.getenv(key="VISION_CAL_SHOW_TIME", default=5)))
    ...
    def cb_send_img_calibrate(self, msg):
        ...
        try:
            img_msg = self.img_bridge.cv2_to_imgmsg(
                cvim=self.cams_caps.camera_handlers[cam_label].image, encoding="bgr8"
            )
            # img_msg.header.stamp = time.time()
            self.pb_img_to_calibrate.publish(img_msg)

        except CvBridgeError as e:
            self.get_logger().error("publishing image to calibrator, {}".format(e))
    ...
    def img_stitch(self, imgs_dic):
        ...
        for idx, img in enumerate(imgs):
            if img is not None:
                if (
                    img.shape[0] != self._VIDEO_HEIGHT
                    or img.shape[1] != self._VIDEO_WIDTH
                ):
                    imgs[idx] = cv2.resize(
                        img,
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                twait = 1.0 / self.cams_config["C"]["FPS"] - tock
                if twait <= 0.0:
                    continue
                time.sleep(twait)
