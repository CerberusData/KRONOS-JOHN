...
class Extrinsic:
    def __init__(self, cam_labels, mtx=None, dist=None):
        ...
        self._CALIBRATION_DAYS_OUT = int(
            os.getenv(key="VISION_CAL_DAYS_OUT", default=10)
        )
        self._CALIBRATION_DAYS_REMOVE = int(
            os.getenv(key="VISION_CAL_DAYS_REMOVE", default=20)
        )
        ...
        for cam_label, params in self.cams.items():
            ...
            if params is not None:
                for param_name, param_value in params.items():
                    value = (
                        ": {}".format(param_value)
                        if type(param_value) in [float, int, tuple, list]
                        else ""
                    )
                    str_ += "\t{}: {}{}\n".format(param_name, type(param_value), value)
            else:
                str_ += "\t{}\n".format(params)
        str_ += "\n"
        return str_
    ...
    def load(self, FILE_NAME, mtx=None, dist=None):
        ...
        if os.path.isfile(abs_path):
            ...
            try:
                ...
                extrinsic_cal["mtime_days"] = (
                    atime - mtime
                ).days  # Last modification date in days
                extrinsic_cal["ctime_days"] = (
                    atime - ctime
                ).days  # Last creation date in days
                extrinsic_cal["out_date"] = {"state": False, "days": 0}

                if (
                    extrinsic_cal["mtime_days"] > self._CALIBRATION_DAYS_OUT
                    or extrinsic_cal["ctime_days"] > self._CALIBRATION_DAYS_OUT
                ):

                    extrinsic_cal["out_date"]["state"] = True
                    extrinsic_cal["out_date"]["days"] = (
                        extrinsic_cal["mtime_days"]
                        if (extrinsic_cal["mtime_days"] > extrinsic_cal["ctime_days"])
                        else extrinsic_cal["ctime_days"]
                    )

                    # Remove calibration file if it's too old and report log
                    if (
                        extrinsic_cal["out_date"]["days"]
                        > self._CALIBRATION_DAYS_REMOVE
                    ):
                        os.remove(abs_path)
                        printlog(
                            msg="{} camera calibration has been removed, "
                            "please re-calibrate cameras now".format(FILE_NAME),
                            msg_type="WARN",
                            flush=self.flush,
                        )
                    else:  # Warning if calibration file is being to old
                        printlog(
                            msg="{} camera calibration will be removed in {}"
                            " days".format(
                                FILE_NAME,
                                self._CALIBRATION_DAYS_REMOVE
                                - extrinsic_cal["out_date"]["days"],
                ...
                extrinsic_cal["waypoint_area"] = get_contour_dist(
                    cnt=[
                        (0, extrinsic_cal["p1"][1]),
                        (extrinsic_cal["image_size"][0], extrinsic_cal["p2"][1]),
                        (
                            extrinsic_cal["image_size"][0],
                            extrinsic_cal["image_size"][1],
                        ),
                ...
                printlog(
                    msg="{} extrinsic parameters loaded".format(FILE_NAME),
                    msg_type="OKGREEN",
                    flush=self.flush,
                )
                ...
                printlog(
                    msg="error while reading {}, {}".format(abs_path, e),
                    msg_type="ERROR",
                    flush=self.flush,
                )
                return None
            ...
            printlog(
                msg="No extrinsic file {}".format(FILE_NAME),
                msg_type="WARN",
                flush=self.flush,
            )
...
def find_projection(
    ...
    dist,
    PATTERN_THRESH_TOP,
    PATTERN_THRESH_BOTTOM,
    HSVI,
    HSVS,
    PATTERN_FILTER_KERNEL=1,
    PATTERN_ITERATION_TRIES=20,
    ...
    img_scr_undistort = cv2.undistort(src=img_src, cameraMatrix=mtx, distCoeffs=dist)

    # Apply Gaussian filter to image
    img_scr_blur = cv2.GaussianBlur(
        src=img_scr_undistort,
        ksize=(PATTERN_FILTER_KERNEL * 2 + 1, PATTERN_FILTER_KERNEL * 2 + 1),
        sigmaX=0,
    )
    ...
    while not solid_pattern and iteration < PATTERN_ITERATION_TRIES:
        ...
        if iteration <= int(PATTERN_ITERATION_TRIES * 0.5):
            ...
            SI_T = HSVI["S"]
            SS_T = HSVS["S"]
            VI_T = HSVI["V"] - 10 * iteration
            VS_T = HSVS["V"]
        else:
            HI_T = HSVI["H"]
            HS_T = HSVS["H"]
            SI_T = HSVI["S"] - 5 * (iteration - 10)
            SS_T = HSVS["S"]
            VI_T = VI_T
            VS_T = HSVS["V"]

        img_scr_hsv = cv2.cvtColor(src=img_scr_blur, code=int(cv2.COLOR_BGR2HSV))
        img_scr_hsv = cv2.inRange(
            src=img_scr_hsv, lowerb=(HI_T, SI_T, VI_T), upperb=(HS_T, SS_T, VS_T)
        )
        img_scr_hsv = cv2.dilate(
            src=img_scr_hsv, kernel=np.ones((3, 3), np.uint8), iterations=1
        )
        ...
        if cv2v_base == "4":
            ...
            _, contours, _ = cv2.findContours(
                image=img_scr_hsv, mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_SIMPLE
            )

        img_scr_hsv_gui = cv2.cvtColor(
            src=img_scr_hsv, code=int(cv2.COLOR_GRAY2BGR)
        )  # Image to debug and test
        img_scr_hsv = np.zeros(
            (IMG_SIZE[1], IMG_SIZE[0], 1), np.uint8
        )  # Mask to draw valid  pattern contours
        ...
        for cnt_idx, cnt in enumerate(contours):
            ...
            if solidity > 0.12 and solidity < 0.25:
                ...
                and solidity < 0.9
                and cnt_area > 1600
                and cnt_perimeter > 300
            ) or (solidity > 0.24 and cnt_area > 800 and cnt_perimeter > 200):
                pattern_sides += 1
                cv2.drawContours(
                    image=img_scr_hsv_gui,
                    contours=[cnt],
        ...
        if not solid_pattern:
            printlog(
                msg="No pattern found in iteration {}".format(iteration),
                msg_type="WARN",
            )
    ...
    img_scr_hsv = cv2.dilate(
        src=img_scr_hsv, kernel=np.ones((kernel, kernel), np.uint8), iterations=1
    )
    ...
    try:
        ...
        singular = (
            Lhs.shape[0] == Lhs.shape[1] and np.linalg.matrix_rank(Lhs) == Lhs.shape[0]
        )
        if not singular:
    ...
    if len(Hy):  # Assign horizontal line Y Cord
        Hy = max(Hy)
        cv2.line(
            img_scr_undistort, (0, int(Hy)), (IMG_SIZE[0], int(Hy)), (255, 179, 255), 2
        )
    else:
        Hy = 0
            ...
            TLI.append(
                line_intersection(
                    ...
                    (
                        ...
                        (IMG_SIZE[0], int(IMG_SIZE[1] * 0.5)),
                    ),
                )[0]
            )
            LLI.append(
                line_intersection(
                    ((x1, y1), (x2, y2)), ((0, IMG_SIZE[1]), (IMG_SIZE[0], IMG_SIZE[1]))
                )[0]
            )
    ...
    if LOCAL_CALIBRATION:
        for point in LLI:
            cv2.line(
                img=img_scr_undistort,
                pt1=(point, IMG_SIZE[1]),
                pt2=(point, IMG_SIZE[1] - 10),
                color=[102, 255, 51],
                thickness=1,
            )
        for point in LRI:
        ...
        for point in TLI:
            cv2.line(
                img=img_scr_undistort,
                pt1=(point, int(IMG_SIZE[1] * 0.5) - 5),
                pt2=(point, int(IMG_SIZE[1] * 0.5) + 10),
                color=[102, 255, 51],
                thickness=1,
            )
        for point in TRI:
    ...
    if LOCAL_CALIBRATION:  # Draw lines which averages were calculated
        if len(new_LRI) and len(new_LLI) and len(new_TRI) and len(new_TLI):
            cv2.line(
                img_scr_undistort,
                (max(new_LRI), IMG_SIZE[1]),
                (min(new_LRI), IMG_SIZE[1]),
                [77, 195, 255],
                2,
            )
            cv2.line(
                img_scr_undistort,
                (max(new_LLI), IMG_SIZE[1]),
                (min(new_LLI), IMG_SIZE[1]),
                [102, 255, 51],
                2,
            )
            cv2.line(
                img_scr_undistort,
                (max(new_TRI), int(IMG_SIZE[1] * 0.5) + 10),
                (min(new_TRI), int(IMG_SIZE[1] * 0.5) + 10),
                [77, 195, 255],
                1,
            )
            cv2.line(
                img_scr_undistort,
                (max(new_TLI), int(IMG_SIZE[1] * 0.5) + 10),
                (min(new_TLI), int(IMG_SIZE[1] * 0.5) + 10),
                [102, 255, 51],
                1,
            )
            cv2.imshow(LOCAL_WIN_NAME, img_scr_undistort)
    ...
    Right_angle = round(
        math.atan2(int(IMG_SIZE[1] * 0.5), TRI - LRI) * (180.0 / math.pi) - 180, 2
    )
    Left_angle = round(
        math.atan2(int(IMG_SIZE[1] * 0.5), TLI - LLI) * (180.0 / math.pi) - 180, 2
    )

    # Draw detected left and right
    LLI_2 = line_intersection(
        ((TLI, int(IMG_SIZE[1] * 0.5)), (LLI, IMG_SIZE[1])),
        ((0, top[0]), (IMG_SIZE[0], top[0])),
    )
    LRI_2 = line_intersection(
        ((TRI, int(IMG_SIZE[1] * 0.5)), (LRI, IMG_SIZE[1])),
        ((0, top[0]), (IMG_SIZE[0], top[0])),
    )
    ...
    Bottom_L = int(
        vp[0][0] + (vp[1][0] - bottom) / math.tan(Left_angle * (math.pi / 180.0))
    )
    Bottom_R = int(
        vp[0][0] + (vp[1][0] - bottom) / math.tan(Right_angle * (math.pi / 180.0))
    )
    ...
    Top_R = int(
        line_intersection(top_line, ((vp[0][0], vp[1][0]), (Bottom_R, bottom)))[0]
    )
    Top_L = int(
        line_intersection(top_line, ((vp[0][0], vp[1][0]), (Bottom_L, bottom)))[0]
    )
...
def draw_surface_projection(
    img, vanishing_point, bottom, top, Bottom_R, Bottom_L, Top_R, Top_L, IMG_SIZE
):
    ...
    cv2.line(
        img_lines,
        (0, int(round(img_lines.shape[0] / 2))),
        (img_lines.shape[1], int(round(img_lines.shape[0] / 2))),
        [0, 255, 255],
        1,
    )
    cv2.line(
        img_lines,
        (int(round(img_lines.shape[1] / 2)), 0),
        (int(round(img_lines.shape[1] / 2)), img_lines.shape[0]),
        [0, 255, 255],
        1,
    )

    # Draw corners and points of interest of geometric projection
    img_lines = cv2.circle(
        img_lines, (vanishing_point[0][0], vanishing_point[1][0]), 5, (0, 255, 0), -1
    )
...
def pixel_relation(
    ...
    inters_L1 = line_intersection(
        ((0, 0), (0, IMG_SIZE[1])), ((p1[0], p1[1]), (p4[0], p4[1]))
    )

    inters_L2 = line_intersection(
        ((0, 0), (0, IMG_SIZE[1])), ((p4[0], p4[1]), (p3[0], p3[1]))
    )

    inters_R1 = line_intersection(
        ((IMG_SIZE[0], 0), (IMG_SIZE[0], IMG_SIZE[1])), ((p2[0], p2[1]), (p3[0], p3[1]))
    )

    inters_R2 = line_intersection(
        ((IMG_SIZE[0], 0), (IMG_SIZE[0], IMG_SIZE[1])), ((p4[0], p4[1]), (p3[0], p3[1]))
    )

    # Get projection of intersection of geometric projection lines
    inters_L1 = get_projection_point_dst(
        coords_src=(inters_L1[0], inters_L1[1], 1), M=M
    )
    inters_L2 = get_projection_point_dst(
        coords_src=(inters_L2[0], inters_L2[1], 1), M=M
    )
    inters_R1 = get_projection_point_dst(
        coords_src=(inters_R1[0], inters_R1[1], 1), M=M
    )
    inters_R2 = get_projection_point_dst(
        coords_src=(inters_R2[0], inters_R2[1], 1), M=M
    )

    # Calculate camera position with intersection between previous lines
    cam_view_coord = line_intersection((inters_L1, inters_L2), (inters_R1, inters_R2))
    cam_view_pix = abs(cam_view_coord[1] - UNWARPED_SIZE[1])

    # -------------------------------------------------------------------------
    # X axis pixels relation [pixels/m]
    Left_Line_proj_P1 = get_projection_point_dst(
        coords_src=(Left_Line[0][0], Left_Line[0][1], 1), M=M
    )
    Left_Line_proj_P2 = get_projection_point_dst(
        coords_src=(Left_Line[1][0], Left_Line[1][1], 1), M=M
    )
    Left_Line_proj = (Left_Line_proj_P1, Left_Line_proj_P2)

    Right_Line_proj_P1 = get_projection_point_dst(
        coords_src=(Right_Line[0][0], Right_Line[0][1], 1), M=M
    )
    Right_Line_proj_P2 = get_projection_point_dst(
        coords_src=(Right_Line[1][0], Right_Line[1][1], 1), M=M
    )
    ...
    img_proj = cv2.warpPerspective(
        src=cv2.undistort(src=img_src, cameraMatrix=mtx, distCoeffs=dist),
        M=M,
        dsize=(UNWARPED_SIZE[0], UNWARPED_SIZE[1]),
    )
    if LOCAL_CALIBRATION:
        cv2.imshow(LOCAL_WIN_NAME, img_proj)
        cv2.waitKey(500)

    # Apply Gaussian filter to image
    filt_kernel_size = 3
    img_proj = cv2.GaussianBlur(
        src=img_proj, ksize=(filt_kernel_size, filt_kernel_size), sigmaX=0
    )
    if LOCAL_CALIBRATION:
        cv2.imshow(LOCAL_WIN_NAME, img_proj)
        cv2.waitKey(500)
    ...
    img_proj_hsv = cv2.cvtColor(
        src=img_proj, code=cv2.COLOR_BGR2HSV
    )  # Change color space to HSV
    img_proj_hsv[0 : UNWARPED_SIZE[1], 0 : Left_Line_proj_P1[0] - 5, :] = 0
    img_proj_hsv[
        0 : UNWARPED_SIZE[1], Right_Line_proj_P2[0] + 5 : UNWARPED_SIZE[0], :
    ] = 0
    img_proj_hsv[0 : int(UNWARPED_SIZE[1] * 0.5), 0 : UNWARPED_SIZE[0], :] = 0
    if LOCAL_CALIBRATION:
        cv2.imshow(LOCAL_WIN_NAME, img_proj_hsv)
        cv2.waitKey(500)
    ...
    if not Hy:
        printlog(
            msg="Hy=0, No pattern's horizontal component found in projection calculation",
            msg_type="WARN",
        )
    ...
    while not ppmy and iterations < PATTERN_ITERATION_TRIES and (not H2 or not H3):
        ...
        if iterations <= int(PATTERN_ITERATION_TRIES * 0.5):
            img_proj_thresh = cv2.inRange(
                src=img_proj_hsv,
                lowerb=(
                    HSVI["H"],
                    HSVI["S"] - 2 * iterations,
                    HSVI["V"] - 15 * iterations,
                ),
        ...
        img_proj_thresh = cv2.erode(src=img_proj_thresh, kernel=kernel, iterations=1)
        ...
        if (
            Hy > int(UNWARPED_SIZE[1] * 0.1)
            and Hy < int(UNWARPED_SIZE[1] * 0.9)
            and (H2 or H3)
        ):
        ...
        if not H2:
            printlog(
                msg="\t\tH2 = 0 - No pattern's horizontal component found for hough lines",
                msg_type="INFO",
            )
        if not H3:
            printlog(
                msg="\t\tH3 = 0 - No pattern's horizontal component found by pixels",
                msg_type="INFO",
            )
        if LOCAL_CALIBRATION:
            cv2.imshow(LOCAL_WIN_NAME, img_proj)
    ...
    if not ppmy:
        printlog(
            msg="Horizontal line in pattern no found, check light conditions and try again",
            msg_type="ERROR",
        )
        if LOCAL_CALIBRATION:
            cv2.waitKey(1000)
    ...
    if ppmy:
        dead_view_distance = cam_view_pix / ppmy  # bots No view area in meters [m]

    # Print some information of pixel relations
    printlog(
        msg="ppx:{}[pix/m], ppy:{}[pix/m], dead_view:{}[m], Hy:{}".format(
            round(ppmx, 2), round(ppmy, 2), round(dead_view_distance, 2), int(Hy)
        ),
        msg_type="INFO",
    )
    ...
    extrinsic_relation = {
        "ppmx": ppmx,
        "ppmy": ppmy,
        "dead_view_distance": dead_view_distance,
        "img_result": img_proj,
    }
...
def create_ruler_mask(flag_img, extrinsic_params, fontScale=0.7):
    ...
    Ruler_Mask = np.zeros(
        (UNWARPED_SIZE[1] + int(round(dead_distance * ppmy)), UNWARPED_SIZE[0], 4),
        np.uint8,
    )

    # Find robot's view cord M, SurfacesPoints
    bot_cord = FindBotViewCord(
        image_size=ORIGINAL_SIZE, M=M, SurfacesPoints=(p1, p2, p3, p4)
    )
    ...
    cv2.line(
        img=Ruler_Mask,
        pt1=(bot_cord[0], 0),
        pt2=(bot_cord[0], Ruler_Mask.shape[0]),
        color=(0, 255, 0, 255),
        thickness=1,
    )
    ...
    if ppmy:
        Max_distance = math.trunc(Ruler_Mask.shape[0] / ppmy)
    else:
        return None, None
    ...
    for i in range(0, Max_distance + 1):
        ...
        cv2.line(
            ...
            pt2=(
                int(round(bot_cord[0] + Ruler_Mask.shape[1] * 0.10)),
                Ruler_Mask.shape[0] - y_pos,
            ),
            color=(0, 255, 0, 255),
            thickness=1,
        )
        cv2.putText(
            ...
            str(i * 100),
            (
                int(round(bot_cord[0] + Ruler_Mask.shape[1] * 0.12)),
                Ruler_Mask.shape[0] - int(round(y_pos + ppmy * 0.02)),
            ),
            cv2.FONT_HERSHEY_SIMPLEX,
            fontScale * 0.5,
            (0, 0, 0, 255),
    ...
    for i in range(0, Max_distance * 11):
        ...
        cv2.line(
            ...
            pt1=(bot_cord[0], Ruler_Mask.shape[0] - y_pos),
            pt2=(
                int(round(bot_cord[0] + Ruler_Mask.shape[1] * 0.10)) - 10,
                Ruler_Mask.shape[0] - y_pos,
            ),
            color=(0, 255, 0, 255),
            thickness=1,
    ...
    for i in range(0, Max_distance + 1):
        ...
        cv2.line(
            img=Ruler_Mask,
            pt1=(
                int(round(bot_cord[0] - Ruler_Mask.shape[1] * 0.04)),
                Ruler_Mask.shape[0] - y_pos,
            ),
        ...
        cv2.putText(
            Ruler_Mask,
            str(int(round((i + 0.5) * 100))),
            (
                int(round(bot_cord[0] - Ruler_Mask.shape[1] * 0.15)),
                int(round(Ruler_Mask.shape[0] - int(round(y_pos + ppmy * 0.07)))),
            ),
            cv2.FONT_HERSHEY_SIMPLEX,
            fontScale * 0.4,
            (0, 0, 0, 255),
    ...
    Inv_Ruler_Mask = cv2.warpPerspective(
        src=Temp_Ruler_Mask,
        M=Temp_M,
        dsize=(ORIGINAL_SIZE[0] * Scale_Factor, ORIGINAL_SIZE[1] * Scale_Factor),
    )
    Inv_Ruler_Mask = cv2.resize(
        src=Inv_Ruler_Mask, dsize=ORIGINAL_SIZE, interpolation=cv2.INTER_AREA
    )

    persp_proc = 1 - 4 / 10.0

    y_pos = int(
        UNWARPED_SIZE[1] + int(round(dead_distance * ppmy)) - int(round(4 * ppmy))
    )
    x_pos = int(round(bot_cord[0]))
    pos = get_projection_point_src(coords_dst=(x_pos, y_pos, 1), INVM=np.linalg.inv(M))
    if flag_img is not None:
        flag_img = cv2.resize(
            src=flag_img,
            dsize=(
                int(flag_img.shape[1] * persp_proc),
                int(flag_img.shape[0] * persp_proc),
            ),
            interpolation=cv2.INTER_AREA,
        )
        Inv_Ruler_Mask = overlay_image(
            l_img=Inv_Ruler_Mask,
            s_img=flag_img,
            pos=(pos[0], pos[1] - flag_img.shape[0]),
            transparency=1,
        )
...
def FindBotViewCord(image_size, M, SurfacesPoints, mtx=None, dist=None):
    ...
    x_dst, y_dst = line_intersection(
        (LSInterc_dst, LIInterc_dst), (RSInterc_dst, RIInterc_dst)
    )

    if mtx is not None and dist is not None:
        x_dst, y_dst = get_distor_point(pt=(x_dst, y_dst), mtx=mtx, dist=dist)
