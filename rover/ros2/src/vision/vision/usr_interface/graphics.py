...
class GraphicInterface:
    def __init__(self, parent_node, cam_labels=["C"]):
        ...
        self.subs_dist_sensors = [
            DistanceSensorSuscriber(parent_node=parent_node, topic_name=topic)
            for topic in [
        ...
        self._VISUAL_OVERLAY_CAMS = int(os.getenv(key="VISUAL_OVERLAY_CAMS", default=1))
        self._VISUAL_WAYPOINT = int(os.getenv(key="VISUAL_WAYPOINT", default=1))
        self._VISUAL_ZOOM = int(os.getenv(key="VISUAL_ZOOM", default=1))
        self._VISUAL_OBJECT_DETECTOR = int(
            os.getenv(key="VISUAL_OBJECT_DETECTOR", default=1)
        )
        self._VISUAL_COMPASS = int(os.getenv(key="VISUAL_COMPASS", default=1))
        self._VISUAL_LAT_CAMS_IDLE_TIME = int(
            os.getenv(key="VISUAL_LAT_CAMS_IDLE_TIME", default=1)
        )
        self._VISUAL_REAR_CAM_IDLE_TIME = int(
            os.getenv(key="VISUAL_REAR_CAM_IDLE_TIME", default=1)
        )
        self._VISUAL_CHASSIS_ERROR = int(
            os.getenv(key="VISUAL_CHASSIS_ERROR", default=1)
        )
        self._VISUAL_DISTANCE_SENSORS = int(
            os.getenv(key="VISUAL_DISTANCE_SENSORS", default=1)
        )
        self._VISUAL_CLIFF_SENSORS = int(
            os.getenv(key="VISUAL_CLIFF_SENSORS", default=1)
        )
        self._GUI_GAME_OVER_SCREEN = int(
            os.getenv(key="GUI_GAME_OVER_SCREEN", default=1)
        )
        self._GUI_STOP_SCREEN = int(os.getenv(key="GUI_STOP_SCREEN", default=1))
        self._GUI_UNDISTORD_AREA = int(os.getenv(key="GUI_UNDISTORD_AREA", default=1))
        self._GUI_NO_CALIBRATION_SCREEN = int(
            os.getenv(key="GUI_NO_CALIBRATION_SCREEN", default=1)
        )
        ...
        # ---------------------------------------------------------------------
        # Graphics components
        self.comp_img_stop_sing = gui_image_overlayed(
            img_path=os.path.join(self._IMGS_PATH, "stop_sing.png")
        )
        self.comp_img_game_over = gui_image_overlayed(
            img_path=os.path.join(self._IMGS_PATH, "game_over.png"), sc_fc=0.3
        )
        self.comp_img_no_calibration = gui_image_overlayed(
            img_path=os.path.join(self._IMGS_PATH, "no_calibration.png")
        )

        self.comp_chassis = gui_chassi_report(subs_chassis=self.sub_chassis)
    ...
    def draw_components(self, imgs_dict):
        ...
        if (
            self._GUI_NO_CALIBRATION_SCREEN
            and self.sub_extrinsic.extrinsic.cams["C"] is None
        ):
    ...
    def draw_intrinsic(self, img, cam_label="C", thickness=2):
        ...
        if cam_label in self.sub_extrinsic.extrinsic.cams.keys():
            ...
            cv2.drawContours(
                image=img,
                contours=[
                    self.sub_extrinsic.extrinsic.cams[cam_label]["undistord_cnt"]
                ],
                contourIdx=0,
                color=(0, 0, 200),
                thickness=thickness,
    ...
    def draw_lateral_cams(self, imgs_dict, switched_cam, show_rcam, show_ltcams):
        ...
        if "RR" in imgs_dict.keys() and show_ltcams:
            insert_image(
                original_image=imgs_dict["P"],
                inserted_image=imgs_dict["RR"],
                new_width=int(imgs_dict["P"].shape[1] * 0.3),
                new_height=int(imgs_dict["P"].shape[0] * 0.3),
                position="ur",
                border_color=(255, 255, 255)
                if self.sub_extrinsic.extrinsic.cams["RR"] is not None
                else (0, 0, 255),
            )
        if "LL" in imgs_dict.keys() and show_ltcams:
            insert_image(
                original_image=imgs_dict["P"],
                inserted_image=imgs_dict["LL"],
                new_width=int(imgs_dict["P"].shape[1] * 0.3),
                new_height=int(imgs_dict["P"].shape[0] * 0.3),
                position="ul",
                border_color=(255, 255, 255)
                if self.sub_extrinsic.extrinsic.cams["LL"] is not None
                else (0, 0, 255),
            )
        if "B" in imgs_dict.keys() and show_rcam:
            insert_image(
                original_image=imgs_dict["P"],
                inserted_image=imgs_dict["B"],
...
class gui_image_overlayed:
    def __init__(self, img_path, transparency=0.5, sc_fc=0.5, x=0, y=0, flip=False):
        ...
        if os.path.isfile(img_path):
            ...
            self._overlayed_mask = cv2.resize(
                src=self._overlayed_mask,
                dsize=(
            ...
            if flip:
                self._overlayed_mask = cv2.flip(src=self._overlayed_mask, flipCode=-1)
        else:
            printlog(
                msg="No file to load image at {}".format(img_path), msg_type="ERROR"
            )
    ...
    def draw(self, img_src, position="cc", x_off=0.0, y_off=0.0):
        ...
        if position == "cc":
            ...
            pos_coord = (
                int(
                    img_src.shape[1] * (0.5 + x_off)
                    - self._overlayed_mask.shape[1] * (0.5 + x_off)
                ),
                int(
                    img_src.shape[0] * (0.5 + y_off)
                    - self._overlayed_mask.shape[0] * (0.5 + y_off)
                ),
            )

        # Overlay game over image text
        img_src = overlay_image(
            l_img=img_src,
            s_img=self._overlayed_mask,
            pos=pos_coord,
...
class gui_chassi_report:
    ...
    def draw(self, img_src):
        ...
        if True in self.subs_chassis.error:
            ...
            if self.wheel_2.transparency > 0.7:
                self.transp = -0.1
            elif self.wheel_2.transparency < 0.2:
                self.transp = 0.1
            ...
            if self.subs_chassis.error[0]:
                self.wheel_1.draw(
                    img_src=img_src, position="", x_off=0.095, y_off=0.255
                )
...
class gui_distance_sensors:
    def __init__(self, subs_sensor_list):
        ...
        self._GUI_SENSORS_DISTANCE_MEASURE = int(
            os.getenv(key="GUI_SENSORS_DISTANCE_MEASURE", default=1)
        )
        self._GUI_SENSORS_DISTANCE_LONG = float(
            os.getenv(key="GUI_SENSORS_DISTANCE_LONG", default=150)
        )

        self._angle_start = int(
            90
            - int(os.getenv(key="GUI_SENSORS_DISTANCE_APERTURE_ANGLE", default=30))
            * 0.5
        )
        self._angle_end = int(
            90
    ...
    def draw_sensor(self, img_src, sensor, idx, x_offset=0):
        ...
        if self._GUI_SENSORS_DISTANCE_MEASURE and sensor.range > sensor.min_range:
            self.draw_text(
                img=img_src,
    ...
    def draw_text(self, img, text, org, color):
        cv2.putText(
            ...
            color=(0, 0, 0),
            thickness=self._font_thickness * 4,
        )
        cv2.putText(
            img=img,
            text=text,
...
class gui_cliff_sensors:
    ...
    def draw_sensor(self, img_src, idx, x_offset=0):
        ...
        center = (idx + self._x_offset + x_offset, img_src.shape[0] + self._y_offset)
