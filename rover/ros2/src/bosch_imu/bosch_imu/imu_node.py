...
class ImuNode(Node):
    ...
    def _update_imu_data(self):
        ...
        if buf != 0:
            ...
            gravity_x = (
                float(st.unpack("h", st.pack("BB", buf[38], buf[39]))[0]) / 100.0
            )
            gravity_y = (
                float(st.unpack("h", st.pack("BB", buf[40], buf[41]))[0]) / 100.0
            )
            gravity_z = (
                float(st.unpack("h", st.pack("BB", buf[42], buf[43]))[0]) / 100.0
            )
            ...
            self.imu_msg.orientation.w = float(
                st.unpack("h", st.pack("BB", buf[24], buf[25]))[0]
            )
            self.imu_msg.orientation.x = float(
                st.unpack("h", st.pack("BB", buf[26], buf[27]))[0]
            )
            self.imu_msg.orientation.y = float(
                st.unpack("h", st.pack("BB", buf[28], buf[29]))[0]
            )
            self.imu_msg.orientation.z = float(
                st.unpack("h", st.pack("BB", buf[30], buf[31]))[0]
            )

            self.imu_msg.linear_acceleration.x = (
                float(st.unpack("h", st.pack("BB", buf[32], buf[33]))[0])
                / self.acc_fact_
                + gravity_x
            )
            self.imu_msg.linear_acceleration.y = (
                float(st.unpack("h", st.pack("BB", buf[34], buf[35]))[0])
                / self.acc_fact_
                + gravity_y
            )
            self.imu_msg.linear_acceleration.z = (
                float(st.unpack("h", st.pack("BB", buf[36], buf[37]))[0])
                / self.acc_fact_
                + gravity_z
            )
            ...
            self.imu_msg.angular_velocity.x = (
                float(st.unpack("h", st.pack("BB", buf[12], buf[13]))[0])
                / self.gyr_fact_
            )
            self.imu_msg.angular_velocity.y = (
                float(st.unpack("h", st.pack("BB", buf[14], buf[15]))[0])
                / self.gyr_fact_
            )
            self.imu_msg.angular_velocity.z = (
                float(st.unpack("h", st.pack("BB", buf[16], buf[17]))[0])
                / self.gyr_fact_
            )
            ...
            self.mag_msg.magnetic_field.x = (
                float(st.unpack("h", st.pack("BB", buf[6], buf[7]))[0]) / self.mag_fact_
            )
            self.mag_msg.magnetic_field.y = (
                float(st.unpack("h", st.pack("BB", buf[8], buf[9]))[0]) / self.mag_fact_
            )
            self.mag_msg.magnetic_field.z = (
                float(st.unpack("h", st.pack("BB", buf[10], buf[11]))[0])
                / self.mag_fact_
            )
